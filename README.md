...
The fan plots — forecast ensemble against the observed trajectory — are built
outside `run_pipeline`, because they read the per-run forecast files directly
rather than the grouped metrics:

| Step | Script | Reads | Writes |
| --- | --- | --- | --- |
| a. Extract ensembles | `src/plotting/export_fan_data.m` | `results/forecasts/` | `results/csv/fan_*.csv` |
| b. Draw | `src/plotting/plot_fan_forecast.R` | `results/csv/fan_*.csv` | `results/figures/fan_*` |
...
src/
  ...
  plotting/            figure scripts (R) + the fan-plot data exporter (MATLAB)
...
**Model runs are not bit-reproducible.** The model uses two random streams and
only one of them can be fixed:

- **MATLAB's** (`rand`, `randi`, `poissrnd` in `model_forecast_run`) is seeded
  with `run_id + SeedOffset`, so that half is deterministic.
- **The one inside `integrate_model`**, which draws every Poisson transition,
  is seeded from the wall clock on each call and cannot be set from MATLAB.
  `integrate_model(seed)` is accepted but ignored.

Repeating a run therefore gives a different stochastic realisation. This is the
behaviour of the model as submitted, and `results/` holds the outputs the
manuscript was written from; re-running the pipeline will reproduce the
findings but not the files byte for byte. Change `SeedOffset` to vary MATLAB's
stream deliberately.

Synthetic truths are seeded the same way, from the digits of the truth id, and
are subject to the same caveat. Regenerating a truth invalidates the runs
...
| `integrate_model.cpp`       | C++ MEX source: one daily time step of the stochastic SEIR model. Daytime/nighttime transmission, commuting flows via`nl`/`part`/`Cave`, Poisson-sampled transitions S→E→Ir/Iu→R. Seeds its engine from the wall clock — see Reproducibility. |
| `integrate_model.mexmaca64` | Pre-compiled MEX binary for Apple Silicon, built from the source beside it. On any other platform run`build_mex` once.                                                                     |
...
- "Monthly Statistics by Origin - Destination 2016" from AFAC, Gobierno de México available via [https://www.gob.mx/cms/uploads/attachment/file/652389/sase-2016-hitorico-10032017.xlsx](https://www.gob.mx/cms/uploads/attachment/file/652389/sase-2016-hitorico-10032017.xlsx)
- "Airline Origin and Destination Survey (DB1B) 2016" form US Bureau of Transportation Statistics available via [https://www.transtats.bts.gov/Tables.asp?QO_VQ=EFI&amp;QO_anzr=Nv4yv0r%FDb4vtv0%FDn0q%FDQr56v0n6v10%FDf748rB%FLQOEO%FM&amp;QO_fu146_anzr=b4vtv0%FDn0q%FDQr56v0n6v10%FDf748rB](https://www.transtats.bts.gov/Tables.asp?QO_VQ=EFI&QO_anzr=Nv4yv0r%FDb4vtv0%FDn0q%FDQr56v0n6v10%FDf748rB%25FLQOEO%25FM&QO_fu146_anzr=b4vtv0%FDn0q%FDQr56v0n6v10%FDf748rB)
- "Air passenger origin and destination, transborder journeys 2016" from Statistics Canada available via [https://doi.org/10.25318/2310025601-eng](https://doi.org/10.25318/2310025601-eng)

### Daily work commuting

Developed and already published in [Perini et al. 2025](https://doi.org/10.1016/j.epidem.2025.100818)

- Canadian 2016 census (Statistics Canada): Commuting Flow from Geography of Residence to Geography of Work [https://www12.statcan.gc.ca/census-recensement/2016/dp-pd/dt-td/Rp-eng.cfm?TABID=4&amp;LANG=E&amp;A=R&amp;APATH=3&amp;DETAIL=0&amp;DIM=0&amp;FL=A&amp;FREE=0&amp;GC=0&amp;GL=-1&amp;GID=1354564&amp;GK=0&amp;GRP=1&amp;O=D&amp;PID=111333&amp;PRID=10&amp;PTYPE=109445&amp;S=0&amp;SHOWALL=0&amp;SUB=0&amp;Temporal=2017&amp;THEME=125&amp;VID=0&amp;VNAMEE=&amp;VNAMEF=%20(2017)&amp;D1=0&amp;D2=0&amp;D3=0&amp;D4=0&amp;D5=0&amp;D6=0](<https://www12.statcan.gc.ca/census-recensement/2016/dp-pd/dt-td/Rp-eng.cfm?TABID=4&LANG=E&A=R&APATH=3&DETAIL=0&DIM=0&FL=A&FREE=0&GC=0&GL=-1&GID=1354564&GK=0&GRP=1&O=D&PID=111333&PRID=10&PTYPE=109445&S=0&SHOWALL=0&SUB=0&Temporal=2017&THEME=125&VID=0&VNAMEE=&VNAMEF=%20(2017)&D1=0&D2=0&D3=0&D4=0&D5=0&D6=0>)
- Canada Frontier Counts (Statistics Canada): Number of vehicles travelling between Canada and the United States [https://www150.statcan.gc.ca/t1/tbl1/en/tv.action?pid=2410000201](https://www150.statcan.gc.ca/t1/tbl1/en/tv.action?pid=2410000201)
- 5-Year American Community Survey (ACS) (United States Census Bureau) Commuting Flows [https://www.census.gov/data/tables/2015/demo/metro-micro/commuting-flows-2015.html](https://www.census.gov/data/tables/2015/demo/metro-micro/commuting-flows-2015.html)
- Mexican Intercensal Survey 2015 (National Institute of Statistics and Geography, INEGI) [https://en.www.inegi.org.mx/programas/intercensal/2015/#Microdatos](https://en.www.inegi.org.mx/programas/intercensal/2015/#Microdatos)

### COVID-19 reported cases

- COVID-19 case data from various public sources available via [https://health.google.com/covid-19/open-data/data-sources](https://health.google.com/covid-19/open-data/data-sources)
...
- **C++ compiler** compatible with MATLAB MEX — only to recompile `integrate_model.cpp`; a pre-compiled Apple Silicon binary is included, other platforms run `build_mex` once

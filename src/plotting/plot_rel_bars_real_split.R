...
#
# Variant of plot_rel_bars_real.R with a split panel layout (supplement figure).
...
# Output: results/figures/rel_bars_real_<target>.{pdf,png}
#
#   Rscript src/plotting/plot_rel_bars_real_split.R
...
out_pdf        <- file.path(ROOT, "results", "figures",
                            paste0("rel_bars_real_", selected_target, ".pdf"))
out_png        <- file.path(ROOT, "results", "figures",
                            paste0("rel_bars_real_", selected_target, ".png"))
  ...
  if (is_left) {
    ...
    p <- ggplot(data_sub,
      ...
      geom_label(aes(y = plot_min + 0.001, label = round(n_samples)),
                 ...
                 linewidth = 0, label.padding = unit(0.1, "lines")) +
      geom_point(

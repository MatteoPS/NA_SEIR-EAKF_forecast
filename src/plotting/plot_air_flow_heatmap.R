# plot_air_flow_heatmap.R
#
# Figure 2: merges the Canadian, Mexican and US air passenger tables into the
# annual non-directional travel matrix across the 96 locations, fits the
# gravity model, and draws the heatmap.
#
# Inputs : data/air_travel/*.csv
# Outputs: results/figures/Matrix-air-flow-tri2.png
#          results/figures/GravitiyModel_loglog.png
#          results/csv/tmp_all_but_CA_CA.csv   (intermediate)
#
#   Rscript src/plotting/plot_air_flow_heatmap.R
...
AIR_DIR <- file.path(ROOT, "data", "air_travel")
FIG_DIR <- file.path(ROOT, "results", "figures")
CSV_DIR <- file.path(ROOT, "results", "csv")

CA_US <- read.csv(file.path(AIR_DIR, "CA_state_pair_tot.csv"))
#doubling it to consider also US_CA (assuming mirrored flow)
CA_US$Total_Passengers = CA_US$Total_Passengers*2

MX_MX_US_CA <-read.csv(file.path(AIR_DIR, "MX_state_pair_tot.csv"))
MX_MX_US_CA$Number_of_Routes = NULL

US_US <- read.csv(file.path(AIR_DIR, "US_bidirectional_state_flows.csv"))
...
write.csv(Tot_no_CA_CA, file.path(CSV_DIR, "tmp_all_but_CA_CA.csv"), row.names = F)
...
coords_pop<-read.csv(file.path(AIR_DIR, "coords_pop.csv"))

#getting the population info from previous paper for gravity model
pops<-read.csv(file.path(AIR_DIR, "statecodes_population.csv")) 
...
ggsave(file.path(FIG_DIR, "GravitiyModel_loglog.png"), plot_fit,width = 16*1.2, height = 9*1.2, units = "cm", bg = "white",dpi = 400)
...
ggsave(file.path(FIG_DIR, "Matrix-air-flow-tri2.png"), heat_plot_tri2,width = 22, height = 20, units = "cm", bg = "white")

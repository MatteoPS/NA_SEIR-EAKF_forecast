...
{
//The one-argument form integrate_model(seed) is accepted and ignored.
//This model seeds its engine from the wall clock at every call (see the
//declaration of `generator` below), so there is no stream to seed from MATLAB.
//The guard exists only so that the call does not read prhs[1..10] out of bounds.
    if (nrhs == 1) {
        return;
    }
    ...
    if (nrhs != 11) {
        mexErrMsgIdAndTxt("integrate_model:nrhs",
            ...
            "alphamap), or a single argument to match the seeding call.");
    }
...
//associate pointers
    ...
    //daytime enter S, E, Iu
    vector<double> SentD(num_loc),EentD(num_loc),IuentD(num_loc);
    //nighttime population,Ir,Iu,S,E,R
    vector<double> NN(num_loc),IrN(num_loc),IuN(num_loc),SN(num_loc),EN(num_loc),RN(num_loc);
    //nighttime enter S, E, Iu
    vector<double> SentN(num_loc),EentN(num_loc),IuentN(num_loc);
    ...
    //compute SentD, EentD and IuentD
    for (i=0; i<num_loc; i++){
        for (j=part[i]+1; j<part[i+1]; j++){
            SentD[(int)nl[j]]=SentD[(int)nl[j]]+Cave[j]*SD[i]/(ND[i]-IrD[i]);
    ...
    for (i=0; i<num_loc; i++){
        for (j=part[i]; j<part[i+1]; j++){
            ...
            ///////////////////////////
            double ESenter=theta*dt1*(C[j]-Ir[j])/ND[(int)nl[j]]*SentD[(int)nl[j]];//incoming S
            double ESleft=theta*dt1*S[j]/(ND[(int)nl[j]]-IrD[(int)nl[j]])*popleft[(int)nl[j]];//outgoing S
            ...
            poisson_distribution<int> distribution7(ESenter);
            int Senter=distribution7(generator);
            poisson_distribution<int> distribution8(ESleft);
            int Sleft=min(distribution8(generator),(int)(S[j]*dt1));
            ...
            /////////////////////
            tempS[j]=max((int)(tempS[j]-expr-expu+Senter-Sleft),0);
            ...
            tempR[j]=max((int)(C[j]-tempE[j]-tempIr[j]-tempIu[j]-tempS[j]),0);
        }
    }
    ...
    //compute SentN, EentN and IuentN
    for (i=0; i<num_loc; i++){
        for (j=part[i]+1; j<part[i+1]; j++){
            SentN[(int)nl[j]]=SentN[(int)nl[j]]+Cave[j]*SN[i]/(NN[i]-IrN[i]);
    ...
    for (i=0; i<num_loc; i++){
        for (j=part[i]; j<part[i+1]; j++){
            ...
            ///////////////////////////
            double ESenter=theta*dt2*C[j]/NN[i]*SentN[i];//incoming S
            double ESleft=theta*dt2*tempS[j]/(NN[i]-IrN[i])*popleft[i];//outgoing S
            ...
            int Senter=distribution7(generator);
            poisson_distribution<int> distribution8(ESleft);
            int Sleft=min(distribution8(generator),(int)(tempS[j]*dt2));
            poisson_distribution<int> distribution9(EEenter);
            int Eenter=distribution9(generator);
            poisson_distribution<int> distribution10(EEleft);
            ...
            /////////////////////
            newS[j]=max((int)(newS[j]-expr-expu+Senter-Sleft),0);
            ...
            dailyIr[j]=max((int)(dailyIr[j]+infr),0);
            dailyIu[j]=max((int)(dailyIu[j]+infu),0);
            newR[j]=max((int)(C[j]-newE[j]-newIr[j]-newIu[j]-newS[j]),0);
        }
    }
    /////////////////////
    return;

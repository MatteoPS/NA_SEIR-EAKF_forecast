...
%
% Runs are NOT bit-reproducible. Only one of the two random streams is under
% our control: MATLAB's. The other lives inside integrate_model, which is the
% version used for the submitted results and seeds itself from the wall clock
% on every call, so repeating a run gives a different stochastic realisation.
% See the Reproducibility section of the README.

paths = setup_paths();

%%%%%%%%%%%%%%%%%random seed
% Fixes MATLAB's stream only (rand/randi/poissrnd in this file). The MEX draws
% its Poisson transitions from a clock-seeded engine that cannot be set from
% here; integrate_model(seed) is accepted but ignored, and is kept so the call
% site still documents the intent.

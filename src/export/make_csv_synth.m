function make_csv_synth()
%MAKE_CSV_SYNTH  Export grouped synthetic forecast metrics to a CSV.
%
%   make_csv_synth()
%
% Reads results/forecast_groups/all_synth_forecast_metrics-pois.mat, expresses
% each group's metric relative to its baseline group, and writes the
% analysis-ready table results/csv/rel_bars_synth.csv (input for
% src/plotting/plot_rel_bars_synth.R).
%
% Run MAKE_FORECAST_GROUP first.
%
% See also MAKE_CSV_REAL.

paths = setup_paths();

%% --- 1. Parameters ---

groups_desc_path   = paths.groups_description;
metrics_file_path  = paths.synth_group_file;
sg_to_plot         = {'sgP'};
out_path           = paths.csv_synth;

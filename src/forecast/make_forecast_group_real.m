function make_forecast_group_real()
%MAKE_FORECAST_GROUP_REAL  Aggregate real-incidence forecast metrics.
%
%   make_forecast_group_real()
%
% Bins the metrics of the four real-incidence runs (601-604) by
% weeks-to-event across all countries and waves and writes
% results/forecast_groups/all_real_forecast_metrics.mat.
%
% Output structure: all_real_data.(Settings).(Target).(BinName)
%
% Run MAKE_FORECAST_METRICS_REAL first.
%
% See also MAKE_FORECAST_GROUP.

paths = setup_paths();

% 1. Setup
sc          = load(paths.statecodes);
statecodes  = sc.statecodes;
truth       = load(paths.real_stats);
...
for i = 1:length(run_ids)
    files_found = dir(fullfile(paths.forecasts, sprintf('*_%d_*_fore_real_stats.mat', run_ids(i))));

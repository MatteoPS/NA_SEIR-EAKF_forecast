...
# ── 0. Load data ───────────────────────────────────────────────────────────────
NAR <- st_read("shape/NEW_politicalboundaries_shapefile/NA_PoliticalDivisions/data/boundaries_p_2021_v3.shp")
...
coordinates_df <- read.csv("coordinatesNAE.csv")
...
commuting_df <- read.csv("final_commuting_matrix_Oct2023.csv", check.names = FALSE)
rownames(commuting_df) <- colnames(commuting_df)




flights_df <- read.csv("Matrix-air-flow-sep04.csv", check.names = FALSE, header = FALSE)
...
ggsave("OUT/plot_NAR_5070_legend.pdf", plot_combined, width = 40, height = 20, units = "in")
system("magick -density 150 OUT/plot_NAR_5070_legend.pdf OUT/plot_NAR_5070_legend.png")

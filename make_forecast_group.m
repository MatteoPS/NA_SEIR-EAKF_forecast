...
clearvars -except runs_description groups_description truth_stats population statecodes
...
opts.DataRange = 'A2';
opts = setvartype(opts, 'RunIDtext', 'string'); % Force ID to string
runs_description = readtable("Runs-description.xlsx", opts);
clear opts

groups_description_filename="Groups-description-pois.xlsx";

output_dir = 'Forecasts_groups';
if ~exist(output_dir, 'dir'), mkdir(output_dir); end
summary_filename = sprintf('%s/0128_all_synth_forecast_metrics-pois.mat', output_dir)


opts_groups = detectImportOptions(groups_description_filename);
...
groups_description = readtable(groups_description_filename, opts_groups);
clear opts_groups cols_to_standardize
...
for sg_i = 1:length(supergroup_names)
    ...
    parfor g_i = 1:num_groups_in_sg
        ...
        for r_i = 1:num_runs_in_group
            ...
            [test_forecast_struct, file_found] = load_forecast_struct(run_id);
...
function [forecast_struct, file_found] = load_forecast_struct(run_id)
forecast_struct = [];
file_found = false;

% Pattern match
forecast_files = dir(['Forecasts/*_' char(run_id) '_*_fore_res_group.mat']);
